...
class BienestarService
{
    ...
    public function insertPreTurnos($params)
    {
        if (!(isset($params['referecia147']))) {
            $params['referecia147'] = null;
        }
        $sqlQuery = "INSERT INTO BienestarAnimal_PreTurno (id_turnero, persona_id,mascota_id,tipoTratamiento,referecia147) VALUES(?,?,?,?,?)";
        $bindParams = [$params['id_turnero'], $params['persona_id'], $params['mascota_id'], $params['tipoTratamiento'], $params['referecia147']];

        $servTurnero = new TurneroService;
        $servTurnero->updateTurnos($params['id_turnero']);
