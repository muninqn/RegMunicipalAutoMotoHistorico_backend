...
class SolicitudService
{
        ...
        foreach ($paramsAnteriores as $key => $value) {
            if (array_key_exists($key, $paramsNuevos)) {
                ...
                if (str_contains($key, "path") || $paramsAnteriores[$key] !== $paramsNuevos[$key]) {
                    if (array_key_exists('wp_admin', $paramsNuevos)) {
                        $sqlQuery = "INSERT INTO RMAMH_SolicitudHistorico (solicitud_id,campo,valor_anterior,valor_nuevo,wp_admin) VALUES(?,?,?,?,?)";
                        $bindParams = [$paramsNuevos['id_solicitud'], $key, $paramsAnteriores[$key], $paramsNuevos[$key], $paramsNuevos["wp_admin"]];
                    } else {
    ...
    public function selectSolicitudes()
    {
        ...
        ORDER BY RMAMH_Solicitud.created_at DESC
        ";
        //WHERE deleted_at is null
        // LEFT JOIN RMAMH_SolicitudHistorico ON RMAMH_Solicitud.id_solicitud = RMAMH_SolicitudHistorico.solicitud_id AND campo='cambio_titularidad'
        $bindParams = [];
    ...
    public function obtenerVecinoSolicitud($idSolicitud)
    {
        $sqlQuery = "SELECT wap_persona FROM RMAMH_Solicitud LEFT JOIN RMAMH_Vecino ON id_vecino=vecino_id WHERE id_solicitud=? AND deleted_at is null";
        //WHERE deleted_at is null
        $bindParams = [$idSolicitud];

        $database = new BaseDatos;
        $database->connect();
        return $database->ejecutarSqlSelect($sqlQuery, $bindParams);
    }
    public function insertSolicitudCambioTitular($idSolicitud, $wapPersonaAnterior, $wapPersonaNuevo, $wapPersonaAdmin)
    {
        $tbCambioTitular = TB_RMAMH_CambioTitular;
        $sqlQuery = "INSERT INTO $tbCambioTitular (solicitud_id,wp_anterior, wp_nuevo,wp_admin) VALUES(?,?,?,?)";
        $bindParams = [$idSolicitud, $wapPersonaAnterior, $wapPersonaNuevo, $wapPersonaAdmin];
        $database = new BaseDatos;
        $database->connect();
        return $database->ejecutarSqlInsert($sqlQuery, $bindParams);
    }
    public function getCambiosTitularidad()
    {
        $tbCambioTitular = TB_RMAMH_CambioTitular;
        $sqlQuery ="SELECT $tbCambioTitular.*
        FROM $tbCambioTitular
        LEFT JOIN RMAMH_Solicitud ON RMAMH_Solicitud.id_solicitud = $tbCambioTitular.solicitud_id
        WHERE RMAMH_Solicitud.deleted_at IS NULL
        ORDER BY $tbCambioTitular.created_at DESC";
        //WHERE deleted_at is null
        // LEFT JOIN RMAMH_SolicitudHistorico ON RMAMH_Solicitud.id_solicitud = RMAMH_SolicitudHistorico.solicitud_id AND campo='cambio_titularidad'
        $bindParams = [];
    ...
    public function getCambiosTitularidadSolicitudID($idSolicitud)
    {
        $tbCambioTitular = TB_RMAMH_CambioTitular;
        $sqlQuery ="SELECT $tbCambioTitular.created_at, vecinoAnterior.Nombre AS vecinoAnteriorNombre,vecinoAnterior.Documento AS vecinoAnteriorDocumento, vecinoNuevo.Nombre AS vecinoNuevoNombre,vecinoNuevo.Documento AS vecinoNuevoDocumento, administrador.Nombre AS administradorNombre,administrador.Documento AS administradorDocumento
        FROM $tbCambioTitular
        LEFT JOIN wapPersonas as vecinoAnterior ON vecinoAnterior.ReferenciaID = $tbCambioTitular.wp_anterior
        LEFT JOIN wapPersonas as vecinoNuevo ON vecinoNuevo.ReferenciaID = $tbCambioTitular.wp_nuevo
        LEFT JOIN wapPersonas as administrador ON administrador.ReferenciaID = $tbCambioTitular.wp_admin
        WHERE solicitud_id=? ORDER BY $tbCambioTitular.created_at DESC";
        //WHERE deleted_at is null
        // LEFT JOIN RMAMH_SolicitudHistorico ON RMAMH_Solicitud.id_solicitud = RMAMH_SolicitudHistorico.solicitud_id AND campo='cambio_titularidad'
        $bindParams = [$idSolicitud];
